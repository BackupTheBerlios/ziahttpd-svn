...
// Last update Wed Nov 16 14:02:26 2005 
//
...
MOD_EXPORT( HK_SEND_RESPONSE)(http::session& session, server::core*, int&)
{
  ...
  // Display data
  {
    dataman::buffer buf((const unsigned char*)"salu ca roule de ton cote moi ca roulemais bon de toute facon c pas non plus le truc ls ",
			strlen("salu ca roule de ton cote moi ca roulemais bon de toute facon c pas non plus le truc ls "));
    session.content_out() = buf;
